...
export default function ThemeConfig({className, theme, setTheme}) {
	const themes = ['Original', 'Blue']
			...
			<Listbox value={theme} onChange={(value) => setTheme(value)}>
				{({ open }) => (
					<div className='flex items-center gap-2'>
						<Label className="block font-semibold text-lg">Tema de la sala:</Label>
						<div className="relative w-32 text-base">
							<ListboxButton className="relative w-full cursor-default rounded-md bg-white py-1 pl-2 pr-10 text-left text-gray-900 shadow-sm ring-1 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
								<span className="flex items-center">
									<span className="ml-3 block truncate">{theme}</span>
								</span>
...
return (
		<div className={`ThemeConfig | ${className} w-full flex flex-col gap-6`}>
			<ThemePop />
		</div>
	)
}
